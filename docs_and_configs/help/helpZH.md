...
- **游戏**：标准 2048 游戏，内置最强 AI 辅助。
- **练习**：查看定式，学习最优走法，支持录制和回放。
- **测验**：测试掌握程度，自动记录错题，支持回放分析和 Verse 回放导入。
- **小游戏**：多种改版小游戏，提供更多挑战。
- **设置**：全局设置、定式计算、高级算法配置。
- **帮助**：帮助文档。
...
- **冷热路径**：输入多个路径时，第一行是热路径，通常应放在速度较快的 SSD 上；后续路径为冷路径，用于存放访问频率较低的数据，适合超大定式跨盘存储。
...
- **BC 算法**：实验性的超大定式计算模式，通过 family 模数拆分数据以降低峰值内存压力。普通使用保持默认模数即可。
...
#### 1. 分层搜索逻辑
为了优化内存效率，局面被划分为不同的“层”。划分依据通常是盘面数字和。
...
EX 和 EXAD 是较新的计算模式，主要目标是减小内存占用并提升计算速度。

### 6.4.1 EX 算法

EX 算法会将相近的局面组织到更紧凑的数据结构中，从而减少重复和存储压力。简单来说，它使用哈希表定位局面分组，并使用位图记录每个分组中实际存在的局面。

它的主要优势是：

- **文件更小**：EX 通常能显著减小中间文件和最终定式文件体积。
- **兼容性较好**：EX 保持与常规算法一致的定式语义，并支持“仅保留最优分支”选项。
- **适用场景**：适合所有中大型常规定式，通常建议开启。

### 6.4.2 EXAD 算法

EXAD 是 EX 与高级算法的结合：它使用高级算法的大数掩码思想处理超大定式，同时使用 EX 的紧凑存储方式降低文件体积和查找压力。

它适合那些本来就需要高级算法处理的超大定式，并进一步改善磁盘占用和读取效率。

## 6.5 BC 算法

BC 是面向超大定式的实验性计算模式。它通过 family 分组和固定模数拆分局面数据，降低生成与回算阶段的峰值内存压力，但会带来更复杂的磁盘 I/O 和参数选择。

BC 模数应使用 13-256 范围内的质数。设置界面会把无效输入自动匹配到附近的合法质数；一般用户建议保持默认值。
...
**A2**: 保留配置和错题本的步骤：
1. 复制 docs_and_configs 目录下的 mistakes_book.pkl 和 config 文件。
2. 粘贴并覆盖到新版本对应路径。
3. 其余配置文件同理。
...
3. **自定义定式**
修改 docs_and_configs/patterns_config.json 自定义定式参数：
...
&nbsp;&nbsp;&nbsp;&nbsp;定式名称中避免出现'_'字符。不要使用全角标点符号。注意json格式中的尾随逗号。
