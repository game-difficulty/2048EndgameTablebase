...
def compress_and_save(ind3, data, output_filename, lvl=1):
    ...
    with open(output_filename, "wb") as f:
        ...
        current_size += bytes_written
        ind3[-1]["f1"] = 0
        segments[c] = (len(ind3) - 1, current_size)
        c += 1
...
def get_segment_pos(ind3, len_data):
    ...
    for i in range(1, len(ind3) - 1):
        ...
        if end - start > 32768:  # 约每32768条打一个包
            segments_info[c]["f0"], segments_info[c]["f1"], segments_info[c]["f2"] = (
                start,
                end,
                i,
            )
...
def compress_and_save_p(ind3, data, output_filename, lvl=1):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
        for worker_idx in range(num_workers):
            ...
            end_idx = (
                (worker_idx + 1) * segment_size
                if worker_idx < num_workers - 1
                else len(segments_info)
            )
...
def trie_compress_progress(path, filename, dtype):
    ...
    book = np.fromfile(
        fullfilepath, dtype=f"uint32,uint8,uint8,uint8,uint8,{np.dtype(dtype).name}"
    )
    ind0, ind1, ind2, ind3 = data_compress_index(book)

    book_ = np.empty(
        len(book), dtype=f"uint32,{np.dtype(dtype).name}"
    )  # 后32位和成功率，后面分块压缩
    book_["f0"] = book["f0"]
    book_["f1"] = book["f5"]
    del book
...
def _search_trie(ind, board):
    board_prefix = [
        np.uint8((board >> np.uint64(56)) & np.uint64(0xFF)),
...
def search_tree(filepath, ind, segments, board):
    ...
    ind3_seg = np.fromfile(
        filepath + "ii", dtype="uint8,uint16", count=high - low + 2, offset=low * 3 - 3
    )  # 这里多读一组

    pos1 = search_block2(
        ind3_seg[1:], np.uint8((board >> np.uint64(32)) & np.uint64(0xFF))
    )  # 最后一个前缀
    if not pos1[1]:
        return 0, 0, 0, np.empty(0, dtype="uint8,uint16")  # 没找到
...
def trie_decompress_search(filepath, board, ind, segments, success_rate_dtype: str):
    ...
    _, val_type, max_scale, zero_val = DTYPE_CONFIG.get(
        success_rate_dtype, DTYPE_CONFIG["uint32"]
    )
...
def restore_book_and_ind3(z_file, s_file, ii_file):
    ...
    book_ = np.frombuffer(
        b"".join(decompressed_blocks), dtype=[("f0", "uint32"), ("f5", "uint32")]
    )

    # 2. 重建ind3
    # 从ii_file中读取压缩后的ind3
    ind3 = np.fromfile(ii_file, dtype=[("f0", "uint8"), ("f1", "uint16")]).astype(
        "uint8,uint64"
    )
