...
import Config
from egtb_core.BoardMaskerAD import ParamType
from egtb_core.BoardMoverAD import decode_board, reverse
from egtb_core.BookSolverAD import (
    remove_died_ad,
    create_index_ad,
    solve_optimal_success_rate,
    solve_optimal_success_rate_arr,
    replace_val,
    dict_fromfile,
    expand_ad,
    dict_tofile,
...
from egtb_core.LzmaCompressor import decompress_with_7z
...
def recalculate_process_ad_c(
    ...
    for i in range(steps - 3, -1, -1):
        started, book_dict2, ind_dict2 = handle_solve_restart_ad_c(
            i, pathname, steps, book_dict2, ind_dict2, started, current_dtype, zero_val
        )
        if not started:
            continue
        ...
        logger.debug(
            f"step {i} done, solving avg {round(_counter / max(_timer, 0.000001) / 2e6, 2)} mbps\n"
        )
            ...
            file.write(
                ",".join(
                    [
                        str(i),
                        str(_counter),
                        str((_max_rate - zero_val) / (max_scale - zero_val)),
...
def iter_ind_dict4(
    ...
    for count_32k in ind_dict0_keys:
        positions_array = np.fromfile(
            os.path.join(folder, f"{count_32k}.i"), dtype="uint64"
        )
        if count_32k < 0:
            derive_size = int(
        ...
        for start_idx in range(0, len(positions_array), chunk_length):
            ...
            if t1 > t0:
                logger.debug(
                    f"step {step}, gen4, count_32k {count_32k}, chunk {start_idx // chunk_length}, "
    ...
    for count_32k in ind_dict0_keys:
        ...
        with open(book_path, "r+b") as f:
            for start_idx in range(0, len(positions_array), chunk_length):
                ...
                if t1 > t0:
                    logger.debug(
                        f"step {step}, gen2, count_32k {count_32k}, chunk {start_idx // chunk_length}, "
                        f"done {round(len(positions_chunk) * derive_size / (t1 - t0) / 1e6, 2)} mbps"
                    )
                _timer += t1 - t0
                _counter += len(positions_chunk) * derive_size
...
def recalculate_ad_c(
    count_32k: np.uint8,
    positions_chunk: ValueType2,
    book_chunk: ValueType1,
    ...
    chunk_count = max(
        min(
            1024, int(len(positions_chunk) * round(np.log2(derive_size + 1)) // 1048576)
        ),
        1,
    )
    ...
    for chunk in range(chunk_count):
        ...
        for k in prange(start, end):
            if derive_size == 1:
                ...
                if is_gen2_step:
                    book_chunk[k][0] = (
                        book_chunk[k][0] * spawn_rate4
                        + success_probability * probability / empty_slots
                    )
                else:
