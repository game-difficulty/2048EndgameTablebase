...
from egtb_core.BoardMaskerAD import (
    validate,
    tile_sum_and_32k_count3,
    mask_board,
    ParamType,
)
from egtb_core.BoardMoverAD import m_move_all_dir, decode_board
from egtb_core.BookGenerator import (
    predict_next_length_factor_quadratic,
    largest_power_of_2,
    update_hashmap_length,
    validate_length_and_balance,
    log_performance,
    initialize_parameters,
    update_parameters,
    harmonic_mean_by_column,
    update_parameters_big,
    allocate_seg,
)
...
from egtb_core.BookSolverADUtils import dict_to_structured_array3, get_array_view3
                    ...
                    if ((t >> np.uint64(4 * i)) & np.uint64(0xF)) == np.uint64(0):
                        ...
                        for newt, mnt in m_move_all_dir(t1):
                            ...
                            is_valid, is_derived, derived_boards = derive(
                                newt,
                                np.uint32(original_board_sum + 2),
                                tiles_combinations_dict,
                                param,
                            )
                            if not is_valid:
                        ...
                        for newt, mnt in m_move_all_dir(t1):
                            ...
                            is_valid, is_derived, derived_boards = derive(
                                newt,
                                np.uint32(original_board_sum + 4),
                                tiles_combinations_dict,
                                param,
                            )
                            if not is_valid:
...
def derive(
    ...
    """
    total_sum, count_32k, pos_32k, board_, tile64_count = tile_sum_and_32k_count3(
        board, param
    )
...
def handle_restart_ad(i, pathname, arr_init, started, d0, d1):
    ...
    if (
        ...
        or (
            os.path.exists(pathname + str(i + 1) + "b")
...
def generate_process_ad(
    ...
    for i in range(1, steps - 1):
        ...
        if len(d0) < segment_size:
            ...
            d1t, d2, hashmap1, hashmap2, counts1, counts2 = gen_boards_ad(
                d0,
                pattern_check_func,
                canonical_func,
                sym_func,
            ...
            length_factor_multiplier = (
                max(counts2) / np.mean(counts2) if np.mean(counts2) > 0 else 1
            )
        ...
        if (i + ini_board_sum % 64 // 2) % 32 == (
            param.small_tile_sum_limit // 2
        ) % 32 + 1:
...
def gen_boards_big_ad(
    ...
    for seg_index in range(segs_count):
        ...
        arr1t, arr2t, hashmap1, hashmap2, counts1, counts2 = gen_boards_ad(
            arr0t,
            pattern_check_func,
            canonical_func,
            sym_func,
