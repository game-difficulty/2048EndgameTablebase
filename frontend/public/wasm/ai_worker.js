// worker.js
import createAICore from './ai_core.js?v=prune-relaxation-20260917';
...
// 2. 鍒濆鍖?WASM 妯″潡
createAICore({ locateFile: (path) => './' + path + '?v=prune-relaxation-20260917' }).then((instance) => {
    console.log("WASM core loaded.");
    ai_core = instance;

    // 濡傛灉鍒濆鍖栧畬鎴愬墠鏈変换鍔¤繘鏉ワ紝鐜板湪鎵ц瀹?
    if (pendingTask) {
...
class CoreAILogic {
    ...
    is_mess(boardArray) {
        // 浣跨敤鐨勬槸鐪熷疄鏁板瓧锛?,4,8...锛夛紝鑰屼笉鏄鏁?0~15
    ...
    calculate_step(ai_player, boardArray, counts) {
        ...
        // 2. 5-tiler 鐗规畩娈嬪眬鍒ゅ畾锛堜弗鏍煎搴?Python 鎷彿浼樺厛绾э級
...
self.onmessage = function (e) {
    if (e.data.type === 'calculate') {
        if (!ai_core) {
            console.log("WASM 灏氭湭灏辩华锛屾殏瀛樹换鍔?..");
            pendingTask = e.data;
            return;
        }
...
// 灏嗚绠楅€昏緫灏佽锛岀‘淇濆垵濮嬪寲鍓嶅悗璋冪敤閫昏緫涓€鑷?
...
function runAI(boardArray, counts, hexStr) {
    ...
    core_ai_logic.time_limit_ratio = current_time_limit_ratio; // 搴旂敤瀹炴椂姣旂巼
