...
const normalizeNamespace = (value) => String(value || 'ranked-run').replace(/[^a-z0-9_-]/gi, '-');

export function createExclusiveRunLock({ namespace = 'ranked-run', onLost } = {}) {
  const lockNamespace = normalizeNamespace(namespace);
  ...
  const resolveFallbackConflict = (lease) => {
    ...
    if (ownerId.localeCompare(lease.ownerId) > 0) loseFallbackLock();
  };
  ...
  const acquireFallback = (runId) => {
    ...
    fallbackKey = `2048tables:${lockNamespace}-lock:${runId}`;
    ...
    try {
      if (typeof BroadcastChannel === 'function') {
        channel = new BroadcastChannel(fallbackKey);
  ...
  const acquireWebLock = (runId, attempt) => new Promise((resolve) => {
    ...
    navigator.locks.request(
      `2048tables:${lockNamespace}:${runId}`,
