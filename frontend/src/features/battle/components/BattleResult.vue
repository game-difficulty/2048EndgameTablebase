<template>
  <div class="battle-result-backdrop">
    <section class="battle-result-panel" role="dialog" aria-modal="true" aria-labelledby="battle-result-title">
      <header>
        <span class="ui-caption font-black uppercase text-text-secondary">{{ $t(mode === 'live' ? 'battle.result.liveKicker' : 'battle.result.kicker') }}</span>
        <h2 id="battle-result-title">{{ $t(resultTitleKey) }}</h2>
...
const statusLabel = (player) => {
  const key = player?.status === 'disqualified' && player?.mode_data?.finish_reason === 'forfeit'
    ? 'battle.playerStatus.forfeited'
    : `battle.playerStatus.${String(player.status || 'disqualified')}`;
  return te(key) ? t(key) : String(player.status || '');
};
</script>
...
.battle-result-row.unranked .battle-result-rank { color: var(--text-secondary); }
