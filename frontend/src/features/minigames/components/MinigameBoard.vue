...
const revealBoardCellsAt = (indices, animate = true) => {
  ...
  const revealedTiles = cells.value
    .filter((cell) => revealed.has(cell.index) && isRenderableTile(cell) && !hidden.has(cell.index))
...
const runMoveAnimation = async () => {
  ...
  activeTiles.value.forEach((tile) => {
    ...
    if (
      distance > 0 &&
      tile.coverSprites?.some((sprite) => isIceCoverSprite(sprite) && !targetSprites.has(sprite))
    ) {
