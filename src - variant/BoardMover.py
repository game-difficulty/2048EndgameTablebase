...
class BoardMoverWithScore:
    ...
    def move_all_dir(self, board: np.uint64):
        ...
        board2 = self.reverse(board)
        return (
            self.move_left(board),  self.move_right(board),
            self.move_up(board, board2), self.move_down(board, board2))
...
class SingletonBoardMover:
    ...
    def __new__(cls, bm_type, *args, **kwargs):
        if bm_type == 2:
            ...
            raise ValueError("Invalid board_mover_type.")
