...
def initialize_parameters(n, pathname, default_length_factor = 3.2):
    """
    初始化长度乘数
    """
    ...
    return length_factor, length_factors, length_factors_list_path, \
        counts2, length_factor_multiplier
...
def generate_process(
    ...
    length_factor, length_factors, length_factors_list_path, \
        counts2, length_factor_multiplier = initialize_parameters(n, pathname)
    ...
    for i in range(1, steps - 1):
        ...
        d1t, d2, hashmap1, hashmap2, counts1, counts2 = \
            gen_boards(d0, bm, pattern_check_func, to_find_func, target,
                       hashmap1, hashmap2, pattern_encoded, n, length_factor)
            ...
            for b in range(start, end):
                ...
                for i in range(16):  # 遍历每个位置
                    if ((t >> np.uint64(4 * i)) & np.uint64(0xF)) == np.uint64(0):
                        ...
                        for newt, new_score in bm.move_all_dir(t1):
                            if newt != t1 and pattern_check_func(newt, pattern_encoded) and new_score < target:
