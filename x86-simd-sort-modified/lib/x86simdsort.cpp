...
template <typename T, bool use_avx512>
XSS_EXPORT_SYMBOL arrsize_t
partition_uint64_array(T *arr, arrsize_t left, arrsize_t right, T pivot)
{
    using vtype = typename std::
            conditional<use_avx512, zmm_vector<T>, avx2_vector<T>>::type;
    using comparator =
            typename std::conditional<false,
                                      Comparator<vtype, true>,
                                      Comparator<vtype, false>>::type;
    constexpr int num_unroll = vtype::partition_unroll_factor;

    T smallest = vtype::type_max();
    T biggest = vtype::type_min();

    arrsize_t partition_index
            = partition_unrolled<vtype, comparator, num_unroll>(
                    arr, left, right, pivot, &smallest, &biggest);

    return partition_index;
}

template XSS_EXPORT_SYMBOL arrsize_t partition_uint64_array<uint64_t, true>(
        uint64_t *arr, arrsize_t left, arrsize_t right, uint64_t pivot);
template XSS_EXPORT_SYMBOL arrsize_t partition_uint64_array<uint64_t, false>(
        uint64_t *arr, arrsize_t left, arrsize_t right, uint64_t pivot);
