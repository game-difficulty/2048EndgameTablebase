...
[[nodiscard]] inline BCBoardEncodedPosition encode_canonical_quadrants_position(
    ...
    const uint64_t nw_sum = family_tile_sum_values != nullptr
        ? bc_quadrant_sum_value_from_tiles(q.nw, *family_tile_sum_values)
        : lut.sum4_value(nw_desc.sum_id);
    const uint64_t ne_sum = family_tile_sum_values != nullptr
        ? bc_quadrant_sum_value_from_tiles(q.ne, *family_tile_sum_values)
        : lut.sum4_value(ne_desc.sum_id);
    const uint64_t sw_sum = family_tile_sum_values != nullptr
        ? bc_quadrant_sum_value_from_tiles(q.sw, *family_tile_sum_values)
        : lut.sum4_value(sw_desc.sum_id);
    const uint64_t se_sum = family_tile_sum_values != nullptr
        ? bc_quadrant_sum_value_from_tiles(q.se, *family_tile_sum_values)
    ...
    if (!bc_min_side_coord_u64(top_sum, bottom_sum, axis.family_unit(), row_coord) ||
        !bc_min_side_coord_u64(left_sum, right_sum, axis.family_unit(), col_coord)) {
        return out;
    }
    ...
    out.row_family = axis.coord_to_id(row_coord);
    out.col_family = axis.coord_to_id(col_coord);
    const uint64_t cid =
        static_cast<uint64_t>(out.row_family) * axis.family_count() +
