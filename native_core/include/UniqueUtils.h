...
inline bool cpu_has_avx_mask_uncached(unsigned int required_ebx, bool require_zmm) {
    unsigned int eax = 0;
    unsigned int ebx = 0;
    unsigned int ecx = 0;
    unsigned int edx = 0;
    ...
    constexpr unsigned long long kXmmYmm =
        0x2ULL | // XMM
        0x4ULL;  // YMM
    constexpr unsigned long long kZmm =
        0x20ULL | // opmask
        0x40ULL | // ZMM_Hi256
        0x80ULL;  // Hi16_ZMM
    const unsigned long long required_xcr0 = require_zmm ? (kXmmYmm | kZmm) : kXmmYmm;
...
inline bool cpu_has_avx512_mask_uncached(unsigned int required_ebx) {
    return cpu_has_avx_mask_uncached(required_ebx, true);
}
#elif defined(_M_X64) || defined(_M_IX86)
inline bool cpu_has_avx_mask_uncached(int required_ebx, bool require_zmm) {
    ...
    if (!(xmm_state && ymm_state && (!require_zmm || zmm_state))) {
        return false;
    }
...
#if defined(__x86_64__) || defined(__i386)
    return cpu_has_avx_mask_uncached(1u << 5, false);
#elif defined(_M_X64) || defined(_M_IX86)
    return cpu_has_avx_mask_uncached(1 << 5, false);
#else
    return false;
#endif
}
...
inline bool cpu_has_avx2() {
    static const bool cached = cpu_has_avx2_uncached();
    return cached;
}
