...
int32_t AIPlayer::search0(uint64_t b) {
  node.fetch_add(4, std::memory_order_relaxed);
...
int32_t AIPlayer::search_branch(uint64_t t, int32_t depth,
  ...
  // 全局 node 计数加 1，并定期通过位运算检查计时（每 1048576 节点）
  uint64_t current_total = node.fetch_add(1, std::memory_order_relaxed);
  if ((current_total & 1048575) == 0) {
...
int32_t AIPlayer::search_ai_player(uint64_t b, int32_t depth,
  ...
  // Wasm 强制单线程：移除 OpenMP Task 逻辑，改用对齐的串行逻辑
  auto moves = BoardMover::s_move_board_all(b);

  for (int i = 0; i < 4; ++i) {
    if (moves[i].is_valid) {
      uint64_t t = moves[i].board;
      int32_t score = moves[i].score;
      int32_t current_depth = depth;

      // 1. 深度启发式削减：仅在 depth < max_d - 2 时生效，输入源使用原始 score
      if (depth < max_d - 2) {
      ...
      int32_t processed_score = process_score(score, depth);
      int32_t temp =
          search_branch(t, current_depth, sum_increment, branch_nodes) + processed_score;
      ...
      if (depth == max_d) {
        top_scores[i] = temp;
        node.fetch_add(branch_nodes, std::memory_order_relaxed);
      }
    }
  }
...
void AIPlayer::start_search(int32_t depth, int32_t timeout_ms) {
  ...
  if (timeout_ms > 0) {
    ...
    deadline =
        std::chrono::steady_clock::now() + std::chrono::hours(1); // 几乎无限
  }
