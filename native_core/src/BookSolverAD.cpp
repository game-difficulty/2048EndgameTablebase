...
std::string ad_solve_stats_header() {
    return "stage,step,input_live,post_zero_live,deletion_threshold,relative_deletion_threshold,"
           ...
           "current_write_seconds,future_compact_seconds,future_write_seconds,compress_seconds,time";
}
...
void dict_tofile(
    ...
    const fs::path folder_path = NativePath::from_utf8(options.pathname + std::to_string(step) + "b");
    ...
    if (compress_indices && options.compress) {
        maybe_do_compress_ad(NativePath::to_utf8_string(folder_path));
    }
}
...
std::vector<int> write_ind_chunked(const IndexStore &ind_dict, const RunOptions &options, int step) {
    const fs::path folder_path = NativePath::from_utf8(options.pathname + std::to_string(step) + "bt");
    if (fs::exists(folder_path)) {
...
template <typename T> uint64_t row_count(const BookStore<T> &book_dict) {
    uint64_t rows = 0U;
    for (int key = bucket_key_min(); key <= bucket_key_max(); ++key) {
        rows += static_cast<uint64_t>(book_dict.at(key).rows);
    }
    return rows;
}

template <typename T>
std::pair<uint64_t, uint64_t> remove_died_ad(
    BookStore<T> &book_dict,
    IndexStore &ind_dict,
    T deletion_threshold
) {
    uint64_t rows_before = 0U;
    uint64_t rows_after = 0U;
    for (int key = bucket_key_min(); key <= bucket_key_max(); ++key) {
        ...
        rows_before += static_cast<uint64_t>(bucket.rows);
        size_t write_row = 0;
        ...
        rows_after += static_cast<uint64_t>(bucket.rows);
...
void iter_ind_dict4(
    ...
    const fs::path folder = NativePath::from_utf8(options.pathname + std::to_string(step) + "bt");
    const FileIOUtils::DirectIoConfig io_config = FileIOUtils::direct_io_config_from_options(options);
...
void recalculate_process_ad_chunked_impl(
    ...
    for (int step = options.steps - 3; step >= 0; --step) {
        ...
        if (NativePath::exists(solved_folder)) {
            if (options.compress) {
        ...
        const fs::path temp_folder = NativePath::from_utf8(options.pathname + std::to_string(step) + "bt");
        const fs::path final_folder = NativePath::from_utf8(options.pathname + std::to_string(step) + "b");
        ...
        record.future_threshold_retained_ratio =
            RuntimeControls::retention_ratio(future_threshold_rows_after, future_threshold_rows_before);
        ...
        total_record.future_threshold_rows_before += record.future_threshold_rows_before;
        total_record.future_threshold_rows_after += record.future_threshold_rows_after;
        total_record.future_threshold_retained_ratio =
            RuntimeControls::retention_ratio(
                total_record.future_threshold_rows_after,
                total_record.future_threshold_rows_before
            );
