...
// 返回的 vector 长度必定为 4，索引 0, 1, 2, 3 分别对应方向 1(左), 2(右), 3(上), 4(下)
std::vector<float> find_best_egtb_move(uint64_t target_board, int table_type) {
    // 初始化 4 个方向的胜率为 0.0f
    std::vector<float> rates(4, 0.0f);
    
    for (int d = 1; d <= 4; ++d) {
        auto [post_board, score] = s_move_board(target_board, d);
        
        // 如果撞墙，保持胜率为 0.0f，直接看下一个方向
